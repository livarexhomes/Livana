        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-6 pb-24 md:pb-6 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Total', value: clients.length, color: 'text-blue-600', bg: 'bg-blue-50' },
                { label: 'Approved', value: approved, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                { label: 'KYC Pending', value: pending, color: 'text-amber-600', bg: 'bg-amber-50' },
                { label: 'Suspended', value: clients.filter(c => c.status === 'suspended').length, color: 'text-orange-600', bg: 'bg-orange-50' },
              ].map(s => (
                <div key={s.label} className="bg-white rounded-2xl border border-gray-100 px-4 py-3.5 shadow-sm">
            ...
            {pending > 0 && statusFilter === 'all' && (
              <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-5 py-3">
                ...
                <p className="text-sm font-semibold text-amber-800 flex-1">
                  {pending} landlord{pending > 1 ? 's' : ''} awaiting KYC approval
                </p>
                <Link href="/admin/kyc" className="text-xs font-bold text-amber-700 hover:text-amber-900 transition-colors">
                  Review →
                </Link>
              </div>
            )}
                  ...
                  <table className="w-full min-w-[560px]">
                    <thead>
                      <tr className="border-b border-gray-100 bg-gray-50/70">
                        <th className="text-left px-5 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest">Landlord</th>
                        <th className="text-left px-5 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest hidden md:table-cell">Contact</th>
                        <th className="text-left px-5 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest hidden lg:table-cell">Location</th>
                        <th className="text-left px-5 py-3 text-[10px] font-black text-gray-400 uppercase tracking-widest hidden sm:table-cell">Props</th>
                          ...
                          <tr key={l.id} className="hover:bg-gray-50/60 transition-colors">
                            ...
                            <td className="px-5 py-3.5 hidden md:table-cell">
                              <div className="space-y-0.5">
                                {l.email && <div className="flex items-center gap-1.5 text-xs text-gray-500"><Mail className="w-3 h-3 text-gray-400" /><span className="truncate max-w-[150px]">{l.email}</span></div>}
                                {l.whatsapp && <div className="flex items-center gap-1.5 text-xs text-gray-500"><Phone className="w-3 h-3 text-gray-400" />{l.whatsapp}</div>}
                              </div>
                            </td>
                            <td className="px-5 py-3.5 hidden lg:table-cell">
                              {l.city
                                ? <div className="flex items-center gap-1.5 text-sm text-gray-600"><MapPin className="w-3.5 h-3.5 text-gray-400 shrink-0" />{l.city}</div>
                                : <span className="text-gray-300">—</span>}
                            </td>
                            <td className="px-5 py-3.5 hidden sm:table-cell">
                              <div className="flex items-center gap-1.5 text-sm font-semibold text-gray-900">
                                <Building2 className="w-3.5 h-3.5 text-gray-400" />{l.property_count}
                              </div>
                            </td>
                            <td className="px-5 py-3.5">
                            ...
                            <td className="px-5 py-3.5">
                              <div className="flex items-center justify-end gap-1.5 flex-wrap">
                                {l.status === 'pending' && (
                                  <>
                                    <Link href="/admin/kyc" className="px-2.5 py-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 text-xs font-semibold rounded-lg transition-colors">Review KYC</Link>
                                    <button type="button" onClick={() => updateStatus(l.id, 'approved')} disabled={processing === l.id}
                                      className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-lg transition-colors disabled:opacity-50">Approve</button>
                                    <button type="button" onClick={() => updateStatus(l.id, 'suspended')} disabled={processing === l.id}
                                      className="px-2.5 py-1 bg-orange-50 text-orange-700 hover:bg-orange-100 text-xs font-semibold rounded-lg transition-colors disabled:opacity-50">Suspend</button>
                                  </>
                                )}
                                {l.status === 'approved' && (
                                  <button type="button" onClick={() => updateStatus(l.id, 'suspended')} disabled={processing === l.id}
                                ...
                                {l.status === 'suspended' && (
                                  <button type="button" onClick={() => updateStatus(l.id, 'approved')} disabled={processing === l.id}
                                    className="px-2.5 py-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 text-xs font-semibold rounded-lg transition-colors disabled:opacity-50">Reinstate</button>
                                )}
                                {l.status === 'not_submitted' && <span className="text-xs text-gray-400 italic">Not submitted</span>}
                                <button type="button" onClick={() => handleDelete(l.id)} disabled={processing === l.id}
                                  className="px-2.5 py-1 bg-red-50 text-red-600 hover:bg-red-100 text-xs font-semibold rounded-lg transition-colors disabled:opacity-50">Delete</button>
                              </div>
                            </td>
                          </tr>
                        )
