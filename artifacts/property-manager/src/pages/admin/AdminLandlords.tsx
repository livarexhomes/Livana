...
function LandlordCard({
  ...
  return (
    <article className="group rounded-xl border border-slate-100 bg-white p-3
      shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-all duration-200
      hover:border-slate-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.07)]">

      <div className="flex items-center gap-3">
        {/* Avatar + identity */}
        <div className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ring-1 ${palette.bg} ${palette.text} ${palette.border}`}>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 min-w-0">
            <span className="truncate text-sm font-semibold text-slate-900">{l.full_name}</span>
            <span className={`shrink-0 inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-semibold ring-1 ${meta.pill}`}>
              {meta.label}
            </span>
          </div>
        ...
        <div className="hidden shrink-0 flex-col items-end gap-0.5 sm:flex">
          <span className="rounded-md bg-slate-50 px-2 py-0.5 text-[11px] font-semibold text-slate-700 ring-1 ring-slate-100">
            {l.property_count} {l.property_count === 1 ? 'property' : 'properties'}
          </span>
          <span className="text-[10px] text-slate-400">Joined {formatDate(l.created_at)}</span>
        </div>
      </div>
      ...
      <div className="mt-2.5 flex items-center gap-1.5 border-t border-slate-50 pt-2.5">
        {l.status === 'pending' && (
          <>
            <Link href="/admin/kyc">
              <button type="button"
                className="h-7 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 text-[11px] font-semibold text-indigo-700 hover:bg-indigo-100 transition-colors">
                Review KYC
              </button>
            </Link>
            <button type="button" disabled={busy} onClick={() => onStatus(l.id, 'approved')}
              className="h-7 rounded-lg bg-emerald-500 px-2.5 text-[11px] font-semibold text-white hover:bg-emerald-600 transition-colors disabled:opacity-40">
              Approve
            </button>
            <button type="button" disabled={busy} onClick={() => onStatus(l.id, 'suspended')}
              className="h-7 rounded-lg border border-orange-200 bg-orange-50 px-2.5 text-[11px] font-semibold text-orange-700 hover:bg-orange-100 transition-colors disabled:opacity-40">
              Suspend
            </button>
          </>
        )}
        {l.status === 'approved' && (
          <button type="button" disabled={busy} onClick={() => onStatus(l.id, 'suspended')}
            className="h-7 rounded-lg border border-amber-200 bg-amber-50 px-2.5 text-[11px] font-semibold text-amber-700 hover:bg-amber-100 transition-colors disabled:opacity-40 flex items-center gap-1.5">
            <ShieldOff className="w-3 h-3" /> Suspend
          </button>
        )}
        {l.status === 'suspended' && (
          <button type="button" disabled={busy} onClick={() => onStatus(l.id, 'approved')}
            className="h-7 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 text-[11px] font-semibold text-emerald-700 hover:bg-emerald-100 transition-colors disabled:opacity-40 flex items-center gap-1.5">
        ...
        <button type="button" disabled={busy} onClick={() => onDelete(l.user_id, l.id)}
          className="h-7 rounded-lg border border-red-100 bg-red-50 px-2.5 text-[11px] font-semibold text-red-500 hover:bg-red-100 transition-colors disabled:opacity-40 flex items-center gap-1.5">
          <Trash2 className="w-3 h-3" /> Delete
        </button>

        {busy && <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-slate-300 border-t-blue-500" />}
      </div>
    </article>
  )
}
            ...
            <div className="mx-auto max-w-7xl">
              ...
              <div className="grid gap-4 xl:grid-cols-[1fr_288px]">
                ...
                <div className="space-y-2.5">
                  ...
                  {/* Filter / search bar */}
                  <div className="flex flex-col gap-2 rounded-xl border border-slate-100 bg-white px-3 py-2 shadow-[0_1px_3px_rgba(0,0,0,0.04)] lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <label className="flex h-8 items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2.5 text-sm focus-within:border-slate-400 focus-within:bg-white transition-all">
                      ...
                      <div className="relative">
                        <select value={sort} onChange={e => setSort(e.target.value)}
                          className="h-8 appearance-none rounded-lg border border-slate-200 bg-white pl-2.5 pr-7 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-300 cursor-pointer">
                  ...
                  {loading ? (
                    ...
                    <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white text-center">
                      ...
                      <p className="mt-0.5 text-xs text-slate-400">Landlords will appear here when they register.</p>
                    </div>
                  ) : filtered.length === 0 ? (
                    <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white text-center">
                      <Users className="mb-2 h-8 w-8 text-slate-200" />
                ...
                <aside className="space-y-2.5">
                  ...
                  {/* Pending queue */}
                  <div className="rounded-xl border border-slate-100 bg-white p-3 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                    ...
                    {topPending.length === 0 ? (
                      ...
                      <div className="space-y-1.5">
                        {topPending.map(item => {
                          ...
                          return (
                            <div key={item.id} className="flex items-center gap-2 rounded-lg border border-slate-50 bg-slate-50/80 p-2 hover:bg-slate-100/70 transition-colors">
                              <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-[10px] font-semibold ring-1 ${pal.bg} ${pal.text} ${pal.border}`}>
                                {getInitials(item.full_name)}
                              </div>
                              <div className="min-w-0">
                                <p className="truncate text-xs font-semibold text-slate-800">{item.full_name}</p>
                                <p className="truncate text-[10px] text-slate-400">{item.city ?? 'No city'}</p>
                              </div>
                              <Link href="/admin/kyc" className="ml-auto shrink-0">
                                <span className="inline-flex h-6 w-6 items-center justify-center rounded-md bg-white text-slate-400 ring-1 ring-slate-200 hover:text-slate-700 transition-colors">
                        ...
                        {pending > 4 && (
                          <Link href="/admin/kyc">
                            <button type="button" className="mt-1 w-full rounded-lg border border-slate-200 bg-white py-1.5 text-xs font-semibold text-slate-500 hover:border-slate-300 hover:text-slate-700 transition-colors">
