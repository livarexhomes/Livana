...
const values = [
  ...
  },
  {
    ...
    icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>,
  },
  {
    title: 'Simplicity',
    description: 'Search, filter, and find — no account required to browse. Renters can contact landlords in one tap.',
    icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
  },
]
...
export default function AboutPage() {
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-gradient-to-br from-[#0a1020] via-[#1a2a3a] to-[#4a7f4d] text-white">
        ...
        <div className="grid sm:grid-cols-2 gap-12 items-center">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={stat.label} className="bg-[#6b9e6e]/10 rounded-2xl p-5 text-center">
                <p className="text-2xl font-bold text-[#4a7f4d]">{stat.value}</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map(v => (
              <div key={v.title} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-3">
                <div className="w-11 h-11 rounded-xl bg-[#6b9e6e]/10 text-[#4a7f4d] flex items-center justify-center">
                  {v.icon}
                </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, i) => (
            <div key={step.title}>
              <div className="w-8 h-8 rounded-full bg-[#6b9e6e] text-white text-sm font-bold flex items-center justify-center mb-3">
                {i + 1}
              </div>
      ...
      <section className="bg-gray-50 border-t border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 text-center">
          ...
          <p className="text-gray-500 mb-8 max-w-md mx-auto">Browse verified listings and connect directly with landlords — no account needed.</p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/listings" className="px-6 py-3 bg-[#6b9e6e] hover:bg-[#4a7f4d] text-white font-semibold rounded-xl transition-colors">
              Browse listings
            </Link>
            <Link href="/contact" className="px-6 py-3 bg-white border border-gray-300 hover:border-gray-400 text-gray-700 font-medium rounded-xl transition-colors">
              Get in touch
            </Link>
          </div>
        </div>
