  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 w-full">
        ...
        {images.length === 0 ? (
          <div className="rounded-2xl overflow-hidden bg-gray-100 flex items-center justify-center" style={{ height: 420 }}>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ height: 420 }}>
            <img src={getSupabaseImageUrl(images[0].storage_path)} alt={property.title} className="w-full h-full object-cover" />
          </div>
        ) : (
          <div className="relative rounded-2xl overflow-hidden" style={{ height: 420 }}>
            <div className={`grid h-full gap-2 ${images.length >= 3 ? 'grid-cols-2' : 'grid-cols-1'}`}>
              {/* Main large image */}
              <div className="relative overflow-hidden cursor-pointer" onClick={() => setImgIndex(0)}>
              ...
              {images.length >= 3 && (
                <div className="grid grid-rows-2 gap-2">
                  {[1, 2].map(i => (
                    <div key={i} className="relative overflow-hidden cursor-pointer" onClick={() => setImgIndex(i)}>
                      {images[i] ? (
            ...
            <button
              ...
              className="absolute bottom-4 right-4 flex items-center gap-2 bg-white text-gray-800 text-sm font-semibold px-4 py-2 rounded-xl shadow-md hover:shadow-lg border border-gray-200 transition-all"
            >
        ...
        {/* Lightbox nav (shown when imgIndex > 0 or user clicks) */}
        {images.length > 1 && imgIndex > 0 && (
          <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={() => setImgIndex(0)}>
            <button onClick={e => { e.stopPropagation(); setImgIndex(i => (i - 1 + images.length) % images.length) }}
              className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors">
              <ChevronLeft className="w-6 h-6" />
            </button>
            <img src={getSupabaseImageUrl(images[imgIndex].storage_path)} alt="" className="max-h-[85vh] max-w-[90vw] object-contain rounded-xl" onClick={e => e.stopPropagation()} />
            <button onClick={e => { e.stopPropagation(); setImgIndex(i => (i + 1) % images.length) }}
              className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors">
              <ChevronRight className="w-6 h-6" />
            </button>
            <button onClick={() => setImgIndex(0)} className="absolute top-4 right-4 text-white/70 hover:text-white text-sm font-medium">✕ Close</button>
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/60 text-sm">{imgIndex + 1} / {images.length}</div>
          </div>
        )}
      </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {property.featured && <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-amber-50 text-amber-700">⭐ Featured</span>}
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 leading-tight mb-2">{property.title}</h1>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { icon: <BedDouble className="w-6 h-6 text-blue-500" strokeWidth={1.8} />, value: property.bedrooms, label: `Bedroom${property.bedrooms !== 1 ? 's' : ''}` },
                { icon: <Bath className="w-6 h-6 text-blue-500" strokeWidth={1.8} />, value: property.bathrooms, label: `Bathroom${property.bathrooms !== 1 ? 's' : ''}` },
                property.area_sqft
                  ? { icon: <Maximize className="w-6 h-6 text-blue-500" strokeWidth={1.8} />, value: `${property.area_sqft.toLocaleString()}`, label: 'sq ft' }
                  : { icon: <MapPin className="w-6 h-6 text-blue-500" strokeWidth={1.8} />, value: property.city, label: 'Location' },
              ].map(({ icon, value, label }) => (
                <div key={label} className="flex flex-col items-center gap-2 p-5 rounded-2xl border border-gray-100 bg-gray-50/60 hover:bg-gray-50 transition-colors">
                  {icon}
            ...
            {property.description && <hr className="border-gray-100" />}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={label} className="flex items-start gap-3 p-3.5 rounded-xl border border-gray-100 bg-gray-50/60">
            ...
            <div>
              ...
              {userRole === 'tenant' && (
                <form onSubmit={handleComment} className="flex gap-3 mb-6">
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center shrink-0 shadow-sm">
                    <span className="text-xs font-bold text-white">{tenantName ? tenantName.split(' ').slice(0,2).map(w=>w[0]).join('').toUpperCase() : 'T'}</span>
                  </div>
              ...
              {(userRole === 'landlord' || userRole === 'admin') && (
                <div className="mb-5 p-3 bg-gray-50 rounded-xl border border-gray-100 text-xs text-gray-400 text-center">Only tenants can post comments.</div>
              )}
                  ...
                  {comments.map(c => {
                    ...
                    return (
                      <div key={c.id} className="flex gap-3">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shrink-0 shadow-sm">
            ...
            <div className="lg:sticky lg:top-24 bg-white rounded-2xl border border-gray-200 shadow-lg overflow-hidden">
              ...
              <div className="p-5 border-b border-gray-100">
                <div className="flex items-baseline gap-2 mb-1">
                  <span className="text-2xl font-black text-gray-900">₦{Number(property.price).toLocaleString()}</span>
                ...
                <div className="flex items-center gap-1.5 text-xs text-gray-400">
                  ...
                  <span className={`font-semibold ${statusCfg.cls} px-1.5 py-0.5 rounded-md text-[10px]`}>{statusCfg.label}</span>
                </div>
              </div>
              ...
              <div className="p-5 space-y-3">
                ...
                {whatsappUrl && (
                  <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-3 w-full px-4 py-3.5 rounded-xl border-2 border-[#25D366]/30 hover:border-[#25D366] bg-[#25D366]/5 hover:bg-[#25D366]/10 transition-all group">
                ...
                {/* Email */}
                <a href={emailUrl}
                  className="flex items-center gap-3 w-full px-4 py-3.5 rounded-xl border-2 border-blue-100 hover:border-blue-400 bg-blue-50/40 hover:bg-blue-50 transition-all group">
                    ...
                    {enquiryOpen && (
                      enquirySuccess ? (
                        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-sm text-green-700 text-center flex flex-col items-center gap-2">
                          ...
                          Enquiry sent! The landlord will respond soon.
                        </div>
                      ) : (
                ...
                <div className="flex gap-2 pt-1">
                  {userRole === 'tenant' && (
                    <button onClick={handleSave} disabled={saving}
                      className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-semibold text-sm transition-all border ${saved ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100'}`}>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {landlord.is_verified
                      ? <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-600 mt-0.5"><CheckCircle className="w-3 h-3" /> Verified landlord</span>
