  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-gray-100">
        ...
        <button
          onClick={close}
          className="ml-auto md:hidden w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
        ...
        {mainNav.map(item => {
          ...
          return (
            <Link key={item.label} href={item.href} onClick={close}
              className={`group flex items-center gap-3.5 px-3 py-3 rounded-xl text-[0.875rem] font-medium transition-colors ${
                active
