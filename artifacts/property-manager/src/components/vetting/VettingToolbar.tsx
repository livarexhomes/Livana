  ...
  return (
    <div ref={ref} className="relative w-full sm:w-44">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
         className="flex h-11 w-full items-center justify-between gap-2 rounded-xl border border-[#d7e0d9] bg-[#eef3ee] px-3 text-xs font-semibold text-[#355248] transition-colors hover:bg-[#e3ece5] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
      ...
      {open && (
        <ul
          role="listbox"
           className="absolute left-0 right-0 top-full z-30 mt-1.5 max-h-72 overflow-y-auto rounded-2xl border border-[#d7e0d9] bg-[#fbfcfa] py-1 shadow-xl dark:border-slate-700 dark:bg-slate-900"
        >
          {tabs.map(tab => {
              ...
              <li key={tab.key}>
                <button
                  ...
                  className={cn(
                    ...
                    tab.key === value
                       ? 'bg-[#e4efe8] text-[#2f7560] dark:bg-blue-950/50 dark:text-blue-300'
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
         className="flex h-11 items-center gap-2 rounded-xl border border-[#d7e0d9] bg-[#eef3ee] px-3 text-xs font-semibold text-[#355248] transition-colors hover:bg-[#e3ece5] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
      ...
      {open && (
        <ul
          role="listbox"
           className="absolute right-0 top-full z-30 mt-1.5 w-44 overflow-hidden rounded-2xl border border-[#d7e0d9] bg-[#fbfcfa] py-1 shadow-xl dark:border-slate-700 dark:bg-slate-900"
        >
